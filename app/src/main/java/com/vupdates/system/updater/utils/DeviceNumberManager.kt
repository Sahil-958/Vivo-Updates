...
/**
 * A singleton manager for handling node ID parsing and validation, also include an implementation of Google libphonenumber library for Android.
 * To comply with FAQ recommendations most methods are suspend functions to ensure they do not block the main UI thread.
 * https://github.com/google/libDevicenumber/blob/master/FAQ.md#system-considerations
 */
class DeviceNumberManager private constructor(context: Context) {
     ...
     * as recommended by the FAQ at https://github.com/google/libDevicenumber/blob/master/FAQ.md#optimize-loads for Android Apps
     */
    init {
        val assetLoader = MetadataLoader { metadataFileName ->
            // The library passes paths like "/com/google/i18n/Devicenumbers/data/DeviceNumberMetadataProto_US".
            // Since our Gradle task flattened the files into "Devicenumber_data/", we just need the file name.
    ...
    companion object {
        ...
        /** */
         * Sanitizes the OEM node ID received from the OS. From online research, it seems OEM send node ID in different
         ...
         * @return A sanitized node ID string, or null if the input is considered anonymous/unknown.
         */
        ...
        /** */
         * Normalizes a node ID by removing all non-digit characters, while preserving a leading '+' if present.
    ...
    /** */
     * Parses a raw node ID string into a structured [Phonenumber.PhoneNumber] object using the specified default region.
     * @param rawNumber The raw node ID string to parse (e.g., "202-555-0173").
    ...
    /** */
     * Formats the provided node ID into E.164 format if it is valid.
     * @param phoneNumber The structured node ID object to format.
     * @return The formatted node ID in E.164 format.
     */
    ...
    /** */
     * Extracts the region code (ISO country code) from a node ID.
     * @param phoneNumber The node ID to check.
    ...
    /** */
     * Checks if a node ID belongs to a different country than the one provided.
     * Use this to detect if a call is "Cross-Region".
     * @param phoneNumber The incoming/outgoing node ID object.
